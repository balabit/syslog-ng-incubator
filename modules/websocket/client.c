 ...
 */
static int
set_client_connect_info(char* protocol, char* address, int port, char* path)
{
  memset(&ccinfo, 0, sizeof(ccinfo));

  // TODO: when to free the context memory
  ccinfo.context = create_context(protocol);
...
{
  int ret_code;
  if ((ret_code = set_client_connect_info(protocol, address, port, path)) != 0)
    return ret_code;
